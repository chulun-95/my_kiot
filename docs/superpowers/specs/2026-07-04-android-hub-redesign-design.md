...
Nút "Đăng xuất" giữ nguyên vị trí/hành vi. Nút BÁN HÀNG (POS) giữ nguyên 100% (icon, text, style, hành vi).
...
| **KHÁC** | Hóa đơn, Báo cáo (chỉ OWNER — giữ role-gating hiện tại), Đổi mật khẩu |
...
| Đổi mật khẩu | "Bảo mật tài khoản" (tĩnh) | — |
...
- Header: đổi từ `AppHeader(title = stringResource(core_hub_title))` sang chuỗi chào ghép theo `user.role`.
- Thêm chuỗi mới vào `strings_core.xml`: 3 tên nhóm mới, 2 dòng chào theo vai trò, các caption tĩnh liệt kê ở mục 2.4, mẫu câu cho caption động (vd `"%1$s sản phẩm · %2$s sắp hết"`).
...
- [ ] `HubScreen` hiển thị đúng 3 nhóm mới, đúng thứ tự card, card đã thu nhỏ theo mục 2.3, dòng chào theo vai trò.
